...
class WPEditableComments{
	...
	function edit($text = 'Edit', $before = '',$after = ''){
      ...
      echo $before.
        '<br/><a class="editable-comment dialog" href="'.get_permalink().$get.'editable-comments='.$comment->comment_ID.'" rel="nofollow" style="float:right; margin-left:10px;"><button class="btn btn-link"> EDIT </button></a> '.$after;
		}
	}
    ...
    if(WPEditableComments::qualifications($comment,'delete') && $delete == 'on'){
      ...
      echo $before.
        '<a class="editable-deletecomment" href="'.get_permalink().$get.'editable-deletecomments='.$comment->comment_ID.'" rel="nofollow" style="float:right;margin-left:10px;">
<button class="btn btn-link"> DELETE </button></a>'.$after;
		}		
	}
